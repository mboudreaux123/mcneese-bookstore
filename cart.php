...
<body>
    <?php
        ...
        if(isset($_GET['pagenocart']))
        {
            $pageno = $_GET['pagenocart'];
        }
        else
        {
            $pageno = 1;
        ...
        $query = "SELECT ItemID FROM mcneesebookstore.cart";  
        ...
        if(isset($_POST['checkout']))
        {
            ...
            $query = '
                INSERT INTO mcneesebookstore.purchasehistory (StudentID, ItemID, Cost, DatePurchased, PurchaseStatus)
                (SELECT c.StudentID, bi.ItemID, BuyPrice, NOW(), "Processing"
                FROM mcneesebookstore.cart AS c
        ...
        // Check if entry way removed
        if(isset($_POST['remove']))
        {
            if(isset($_POST['CartID']))
            {
                $query = " 
                    DELETE FROM mcneesebookstore.cart WHERE CartID = " . $_POST['CartID'] . "";
                $result = mysqli_query($connection, $query);  
            }
        }

        $query = " SELECT c.CartID, c.StudentID, b.ItemID, b.Title, b.Rating, inv.BuyPrice, inv.ImagePath, a.Authors
        FROM mcneesebookstore.cart AS c
        JOIN mcneesebookstore.bookinfo AS b ON c.ItemID = b.ItemID
        ...
        WHERE c.StudentID = " . $_SESSION['StudentID'] . " ";

        // Check what to order by from drop down menu
        if(isset($_POST['selector']))
        {
            $_SESSION['cart_selector'] = $_POST['selector'];

            if($_SESSION['cart_selector'] === "az")
            {
                $query .= " ORDER BY Title ASC ";
            }
            else if($_SESSION['cart_selector'] === "za")
            {
                $query .= " ORDER BY Title DESC ";
            }
            else if($_SESSION['cart_selector'] === "major")
            {
                $query .= " ORDER BY Major ASC ";
            }
            else if($_SESSION['cart_selector'] === "lowestrating")
            {
                $query .= " ORDER BY Rating ASC ";
            }
            else if($_SESSION['cart_selector'] === "highestrating")
            {
                $query .= " ORDER BY Rating DESC ";
            }
            else if($_SESSION['cart_selector'] === "lowestprice")
            {
                $query .= " ORDER BY BuyPrice ASC ";
            }
            else if($_SESSION['cart_selector'] === "highestprice")
            {
        ...
        <div class="store-control">
            ...
            <div class="store-control-center">
                <div class="pagination">
                        <a href="?pagenocart=1">First</a>
                        <a href="<?php if($pageno <= 1){ echo '#'; } else { echo "?pagenocart=".($pageno - 1); } ?>">Prev</a>
                        <a Prev><?php echo $pageno; ?></a>
                        <a href="<?php if($pageno >= $total_pages){ echo '#'; } else { echo "?pagenocart=".($pageno + 1); } ?>">Next</a>
                        <a href="?pagenocart=<?php echo $total_pages; ?>">Last</a>
                  </div>
            </div>
            <div class="store-control-right">
                <div class="store-control-right-sub">
                <form method="post" action="cart.php">
                    <button class="checkout-button store-right-item" type="submit" name="checkout">Checkout</button> 
                </form>
                <form method="post" action="cart.php">
                    <select class="selector" name="selector" onChange="this.form.submit()">
                    <?php  if (!isset($_SESSION['cart_selector']) or $_SESSION['cart_selector'] === "") : ?>
                        ...
                        <?php endif ?>
                        <?php  if ($_SESSION['cart_selector'] === "az") : ?>
                            ...
                            <option value="highestprice" name="highestprice">Highest Price</option>
                        <?php endif ?>
                        <?php  if ($_SESSION['cart_selector'] === "za") : ?>
                            <option value="az" name="az">A - Z</option>
                            ...
                            <option value="highestprice" name="highestprice">Highest Price</option>
                        <?php endif ?>
                        <?php  if ($_SESSION['cart_selector'] === "major") : ?>
                            <option value="az" name="az">A - Z</option>
                            <option value="za" name="za">Z - A</option>
                            ...
                            <option value="highestprice" name="highestprice">Highest Price</option>
                        <?php endif ?>
                        <?php  if ($_SESSION['cart_selector'] === "lowestrating") : ?>
                            <option value="az" name="az">A - Z</option>
                            <option value="za" name="za">Z - A</option>
                            <option value="major" name="major">Major</option>
                            ...
                            <option value="highestprice" name="highestprice">Highest Price</option>
                        <?php endif ?>
                        <?php  if ($_SESSION['cart_selector'] === "highestrating") : ?>
                            <option value="az" name="az">A - Z</option>
                            <option value="za" name="za">Z - A</option>
                            <option value="major" name="major">Major</option>
                            <option value="lowestrating" name="lowestrating">Lowest Rating</option>
                            ...
                            <option value="highestprice" name="highestprice">Highest Price</option>
                        <?php endif ?>
                        <?php  if ($_SESSION['cart_selector'] === "lowestprice") : ?>
                            <option value="az" name="az">A - Z</option>
                            <option value="za" name="za">Z - A</option>
                            <option value="major" name="major">Major</option>
                            <option value="lowestrating" name="lowestrating">Lowest Rating</option>
                            ...
                            <option value="highestprice" name="highestprice">Highest Price</option>
                        <?php endif ?>
                        <?php  if ($_SESSION['cart_selector'] === "highestprice") : ?>
                            <option value="az" name="az">A - Z</option>
                            <option value="za" name="za">Z - A</option>
                            <option value="major" name="major">Major</option>
                            <option value="lowestrating" name="lowestrating">Lowest Rating</option>
                ...
                <div class="card">
                    ...
                    <div class="card-buttons">
                        <form method="post" action="cart.php">
                            ...
                            <button class="card-remove-button" type="submit" name="remove">Remove from cart</button>
                        </form>
                    </div>
                </div>
                ';
